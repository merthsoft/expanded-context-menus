...
static class ReverseCommandsCompatibilityWrapper {
    ...
    private static class Types {
        ...
        private static bool CanLoad(string typeName, ref Type value) {
            ...
            if (value == null) {
                Log.Error($"Reverse Commands Compatibility not enabled: Failed to get type {typeName}.");
                return false;
            }
            return true;
        }
    ...
    private static class Methods {
        ...
        private static bool CanLoad(Type type, string methodName, ref Traverse value, params Type[] paramTypes) {
            ...
            if (value == null) {
                Log.Error($"Reverse Commands Compatibility not enabled: Failed to get method {methodName}({string.Join(", ", paramTypes.Select(t => t.Name))}) on type {type.Name}.");
                return false;
            }
            return true;
        }
    ...
    private static class Fields {
        ...
        public static bool Load() {
            ...
            if (LabelMenu == null) {
                Log.Error($"Reverse Commands Compatibility not enabled: Failed to get field named {LabelMenuFieldName} on type {Types.Tools.Name}.");
                return false;
            }
            return true;
        }
    ...
    public static void AttemptEnableCompatibility() {
        if (!LoadedModManager.RunningModsListForReading.Exists(m => m.PackageId.Equals(PackageId, StringComparison.InvariantCultureIgnoreCase))) {
            Log.Message($"Reverse Commands Compatibility not enabled: No mod with package id {PackageId} was found.");
            return;
        }
        ...
        if (Types.Load() && Methods.Load() && Fields.Load()) {
            ...
            Log.Message("Reverse Commands Compatibility enabled.");
            return;
        }
    }
    ...
    public static FloatMenu GetFloatMenu(List<FloatMenuOption> options, string labelCap) {
        ...
        if (CompatibilityEnabled) {
            try {
                ...
                Log.Error($"Reverse Commands Compatibility disabled: An exception occurred in GetFloatMenu: " + ex);
    ...
    public static IEnumerable<FloatMenuOption> GetMenuItems() {
        if (!CompatibilityEnabled) { return Enumerable.Empty<FloatMenuOption>(); }
        ...
        try {
            ...
            Log.Error($"Reverse Commands Compatibility disabled: An exception occurred in GetMenuItems: " + ex);
            CompatibilityEnabled = false;
